...
def encode(key_path: str, message: str):
    ...
    for line in key_file:
        key_file_contents.append((line[4:].replace("\n", ""), line[0]))
...
def generate25():
    return ''.join(
        random.choice(
            "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ2034567890!@#$%^&*()-=_+{}[];\'\",.<>/|"
...
def makeKey():
    ...
    for character in functional_characters:
        file_key.append(
            character + generate4() + generate25() + "?" + generate25() + "?" + generate25() + "?" + generate25() + "?" + generate25() + "?" + generate25() + "?" + generate25() + "?" + generate25())
