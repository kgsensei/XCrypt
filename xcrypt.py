...
GEN_CHARSET: str = ("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ2034567890!@#$%^&*()-=_+{}[];\'\",.<>/|")
...
def generate25() -> str:
    ...
    return ''.join(random.choice(GEN_CHARSET) for _ in range(random.randint(15, 25)))

def generate4():
    """Generates a sequence of 4 random characters."""
    return ''.join(random.choice(GEN_CHARSET) for _ in range(4))
